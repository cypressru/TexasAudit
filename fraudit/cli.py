"""
Fraudit CLI - Command Line Interface

Entry point for all Fraudit operations.
"""
...
@click.group()
@click.version_option(version=__version__, prog_name="fraudit")
@click.pass_context
def cli(ctx):
    """Fraudit - Government Spending Fraud Detection System.
...
def init(ctx, drop):
    ...
    from fraudit.database import init_db, drop_db, get_engine

    click.echo("Initializing Fraudit database...")
...
def show_config(show):
    ...
    if show:
        ...
        click.echo("Use 'fraudit config --show' to view current settings.")
...
def sync_run(source, full, sync_all):
    ...
    from fraudit.ingestion.runner import run_sync
...
def sync_status():
    ...
    from fraudit.database import get_session, SyncStatus
    ...
    with get_session() as session:
        ...
        if not statuses:
            click.echo("No sync history found. Run 'fraudit sync run' to start.")
            return
...
def alerts_show(alert_id):
    """Show alert details."""
    from fraudit.database import get_session, Alert
...
def alerts_acknowledge(alert_id):
    ...
    from fraudit.database import get_session, Alert, AlertStatus
...
def pia_list(status):
    """List PIA requests."""
    from fraudit.database import get_session, PIARequest, PIAStatus
...
def pia_draft(alert, agency, subject):
    ...
    from fraudit.pia import create_draft
    ...
    click.echo("Edit with 'fraudit pia edit {request_id}' or submit with 'fraudit pia submit {request_id}'")
...
def pia_show(request_id):
    ...
    from fraudit.database import get_session, PIARequest
...
def tui():
    ...
    from fraudit.tui import FrauditApp
...
def scheduler(foreground):
    ...
    from fraudit.ingestion.scheduler import start_scheduler
