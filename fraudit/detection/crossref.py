...
from fraudit.database import get_session, Vendor, Alert, AlertSeverity, AlertStatus
...
try:
    from fraudit.database import Employee, CampaignContribution, TaxPermit, LobbyingActivity, EntityMatch
...
def detect_vendor_address_clusters(thresholds: dict) -> int:
    ...
    with get_session() as session:
        ...
        for address, city, state, count in tqdm(address_counts, desc="Address clusters"):
            ...
            from fraudit.database import Payment, Contract
            total_payments = 0
            total_contracts = 0
...
def detect_unregistered_vendors(thresholds: dict) -> int:
    ...
    with get_session() as session:
        ...
        from fraudit.database import Payment
...
def build_entity_network(vendor_id: int) -> dict:
    ...
    with get_session() as session:
        ...
        # Get contracts
        from fraudit.database import Contract
